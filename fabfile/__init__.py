...
def setup(project_name=False, project_type='git', **kwargs):
    ...
    run(
        'trac-admin /var/local/lib/trac/%(project_type)s-%(project_name)s/ permission remove authenticated \'*\'' % cmd_vars)
    run(
        'trac-admin /var/local/lib/trac/%(project_type)s-%(project_name)s/ permission remove anonymous \'*\'' % cmd_vars)
...
def _validate_project_name_and_type(project_name=False, project_type='git', **kwargs):
    ...
    if not m:
        return (False,
                'Valid project names can contain only letters and numbers. Hyphens and periods are allowed, but not as a first or last character.')
...
def _adduser(username=False, project_name=False, project_type='git'):
    ...
    print(
        'Finished adding user, creating group, and setting permissions for the `%(project_name)s` project.' % cmd_vars)
